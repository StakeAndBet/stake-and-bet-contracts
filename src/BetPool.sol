...
import {Ownable} from "playpen/lib/Ownable.sol";
import {FullMath} from "playpen/lib/FullMath.sol";
import {Multicall} from "playpen/lib/Multicall.sol";
import {SelfPermit} from "playpen/lib/SelfPermit.sol";

import {BetToken} from "./BetToken.sol";

contract BetPool is Multicall, SelfPermit, Ownable {
  /// -----------------------------------------------------------------------
  /// Library usage
  /// -----------------------------------------------------------------------
  ...
  constructor(address _betToken, uint64 _duration) {
  ...
  function exit() external {
    ...
    // transfer stake
    betToken.safeTransfer(msg.sender, accountBalance);
    emit Withdrawn(msg.sender, accountBalance);

    // transfer rewards
    if (reward > 0) {
      betToken.safeTransfer(msg.sender, reward);
