...
contract BetManager is AccessControl {
    ...
    function endBettingSession(
        ...
        require(
            sessionRequestId != 0,
            "BetManager: Requesting tweet count failed"
        );
    }

    function addVerifiedTwitterUserId(
        string calldata verifiedTwitterUserId
    ) external onlyRole(BETTING_SESSION_MANAGER_ROLE) {
    ...
    function removeVerifiedTwitterUserId(
        string calldata verifiedTwitterUserId
    ) external onlyRole(BETTING_SESSION_MANAGER_ROLE) {
        require(
    ...
    function recordOneUserBet(
        ...
        users[user].units += ((TOKEN_AMOUNT_PER_BET * UNITS_PER_TOKEN) /
            1 ether);
    ...
    function settleSession(
        bytes32 sessionId,
        uint256 betResult
    ) external onlyRole(BETTING_SESSION_SETTLER_ROLE) returns(bool){
        BettingSession memory session = bettingSessions[sessionId];
        require(
            session.state == SessionState.RESULT_REQUESTED,
        ...
        )
    {
        ...
        tokensForStacking =
            (totalTokensToShare * STACKING_SHARE) /
            SHARE_DIVISOR;
    ...
    function generateSessionId(
        ...
        return
            keccak256(
                abi.encodePacked(twitterUserId, startTimestamp, endTimestamp)
            );
    }
}
